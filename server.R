...
server <- function(input, output, session) {
  ...
  output$HQIIrawdata <- renderRHandsontable({
    rhandsontable(values$HQIIworking)
  })
  ...
  output$HQEDrawdata <- renderRHandsontable(
    rhandsontable(values$HQEDworking)
  )
  ...
  output$HQSMSrawdata <- renderRHandsontable(
    rhandsontable(values$HQSMSworking)
  )
  
  HQtable1test <- reactive({ 
    HQEDdisplay <- values$HQEDworking %>%
      ungroup() %>%
    ...
    return(HQEDdisplay[,c("Event.or.distribution.type.","Q1.Metric","Q1","Q2.Metric", "Q2",
  ...
  HQtable1test1 <- reactive({ 
    HQSMSdisplay <- values$HQSMSworking %>%
      ungroup() %>% 
    ...
    return(HQSMSdisplay[,c("Event.or.distribution.type.","Q1.Metric","Q1","Q2.Metric", "Q2",
                           "Q3.Metric","Q3","Q4.Metric", "Q4", "FY.Metric", "FY.Total")])
    
  })
  
  HQfinal <- reactive(
    rbind(HQtable1test(), HQtable1test1())
  )
  
  
  output$HQtable1 <- renderDataTable(as.datatable(formattable(HQfinal(), list(
    Q1 =  formatter("span",
  ...
  output$downloadHQII <- downloadHandler(
    ...
    content = function(file) {
      write.csv(testthing0(), file, row.names = FALSE)
    }
  )
  output$downloadHQED <- downloadHandler(
    ...
    content = function(file) {
      write.csv(testthing1(), file, row.names = FALSE)
    }
  ) 
  output$downloadHQSMS <- downloadHandler(
    ...
    content = function(file) {
      write.csv(testthing2(), file, row.names = FALSE)
    }
  )
  ...
  output$PBIIrawdata <- renderRHandsontable({
    rhandsontable(values$PBIIworking)
  })
  testthing3 <- reactive({
  ...
  output$PBEDrawdata <- renderRHandsontable(
    rhandsontable(values$PBEDworking)
  )
  testthing4 <- reactive({
  ...
  output$PBSMSrawdata <- renderRHandsontable(
    rhandsontable(values$PBSMSworking)
  )
  ...
  PBtable1test <- reactive({ 
    ...
    PBEDdisplay$Q1.Metric <- c(5,6,0,30,5,1,0,12,2,1,50,0,1,50)
    PBEDdisplay$Q2.Metric <- c(4,6,0,30,5,1,1,12,2,1,50,0,1,50)
    PBEDdisplay$Q3.Metric <- c(2,6,1,30,5,2,0,12,2,1,50,0,1,50)
    PBEDdisplay$Q4.Metric <- c(2,6,0,30,5,2,1,12,2,1,50,0,1,50)
  ...
  PBfinal <- reactive(
    rbind(PBtable1test(), PBtable1test1())
  )
  
  output$PBtable1 <- renderDataTable(as.datatable(formattable(PBfinal(), list(
    Q1 =  formatter("span",
                    style = ~ style(color = ifelse(Q1 < Q1.Metric, "red" , NA))),
    Q2 =  formatter("span",
                    style = ~ style(color = ifelse(Q2 < Q2.Metric, "red" , NA))),
  ...
  output$downloadPBII <- downloadHandler(
    ...
    content = function(file) {
      write.csv(testthing3(), file, row.names = FALSE)
    }
  )
  output$downloadPBED <- downloadHandler(
    ...
    content = function(file) {
      write.csv(testthing4(), file, row.names = FALSE)
    }
  ) 
  output$downloadPBSMS <- downloadHandler(
    ...
    content = function(file) {
      write.csv(testthing5(), file, row.names = FALSE)
    }
  )
  ...
  output$BGIIrawdata <- renderRHandsontable({
    rhandsontable(values$BGIIworking)
  })
  testthing6 <- reactive({
  ...
  output$BGEDrawdata <- renderRHandsontable(
    rhandsontable(values$BGEDworking)
  )
  testthing7 <- reactive({
  ...
  output$BGSMSrawdata <- renderRHandsontable(
    rhandsontable(values$BGSMSworking)
  )
  ...
  BGtable1test <- reactive({ 
    ...
    BGEDdisplay$Q1.Metric <- c(4,3,1,0,26,12,1,1,1,12,4,1,0,4,6)
    BGEDdisplay$Q2.Metric <- c(4,4,1,0,35,12,1,1,1,12,4,1,0,4,6)
    BGEDdisplay$Q3.Metric <- c(4,2,1,0,35,12,0,1,1,12,4,1,0,4,6)
    BGEDdisplay$Q4.Metric <- c(4,4,1,2,35,0,2,1,1,12,4,1,0,4,6)
  ...
  BGfinal <- reactive(
    rbind(BGtable1test(), BGtable1test1())
  )
  
  output$BGtable1 <- renderDataTable(as.datatable(formattable(BGfinal(), list(
    Q1 =  formatter("span",
                    style = ~ style(color = ifelse(Q1 < Q1.Metric, "red" , NA))),
    Q2 =  formatter("span",
                    style = ~ style(color = ifelse(Q2 < Q2.Metric, "red" , NA))),
