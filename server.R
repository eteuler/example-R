...
#save button will read the raw data and create the dashboard with potentially edited cells    
  observeEvent(input$saveHQall, {
    ...
    HQEDMetrics$FY.Metric <- (HQEDMetrics$Q1.Metric + HQEDMetrics$Q2.Metric + 
                                HQEDMetrics$Q3.Metric + HQEDMetrics$Q4.Metric)
    

    
    gdata::unknownToNA(x=HQEDdisplay$Q1, unknown = NA)
    gdata::unknownToNA(x=HQEDdisplay$Q2, unknown = NA)
    gdata::unknownToNA(x=HQEDdisplay$Q3, unknown = NA)
    gdata::unknownToNA(x=HQEDdisplay$Q4, unknown = NA)
...
# })
 ...
 #reading in and celaning raw data for only pueblo INdividual interaction data 
  PBIIdata <- observe({  
    ...
    if (is.null(inFile))
      return(NULL)
    PBIIfinal <- cleanPBII(inFile$datapath, prev.resp.id=last.max.pb) 
 ...
 #call and display raw II pueblo  data  
  output$PBIIrawdata <- renderRHandsontable({
    rhandsontable(rbind(PBInitialList$II,values$PBIIworking), useTypes = FALSE, rowHeaders = FALSE)
  })
...
#reading in and celaning raw data for only INdividual interaction data   
  PBEDdata <- observe({
    ...
    if (is.null(inFile))
      return(NULL)
    PBEDfinal <- cleanPBED(inFile$datapath, prev.resp.id=last.max.pb) 
...
#call and display raw Event and Dist, pueblo data  
  output$PBEDrawdata <- renderRHandsontable(
    rhandsontable(rbind(PBInitialList$ED,values$PBEDworking), useTypes = FALSE, rowHeaders = FALSE)
  )
...
#readind in and cleaning of the social media submission peublo data   
  PBSMSdata <- observe({
    ...
    if (is.null(inFile))
      return(NULL)
    PBSMSfinal <- cleanPBSMS(inFile$datapath, prev.resp.id=last.max.pb)
...
#displaying of SMS raw pueblo data  
  output$PBSMSrawdata <- renderRHandsontable(
    rhandsontable(rbind(PBInitialList$SMS,values$PBSMSworking), useTypes = FALSE, rowHeaders = FALSE)
  )
...
#reading in and cleaning or individual interactions raw data blue grass  
  BGIIdata <- observe({
    ...
    if (is.null(inFile))
      return(NULL)
    print(last.max.bg)
    BGIIfinal <- cleanBGII(inFile$datapath, prev.resp.id=last.max.bg)
...
#displaying of raw II blue grass data  
  output$BGIIrawdata <- renderRHandsontable({
    rhandsontable(rbind(BGInitialList$II,values$BGIIworking), useTypes = FALSE, rowHeaders = FALSE)
  })
...
#reading and cleaning of blue grass Event or Dist. rawdata     
  BGEDdata <- observe({
    ...
    if (is.null(inFile))
      return(NULL)
    BGEDfinal <- cleanBGED(inFile$datapath, prev.resp.id=last.max.bg)
...
#displying of blue grass event or dist raw data 
  output$BGEDrawdata <- renderRHandsontable(
    rhandsontable(rbind(BGInitialList$ED,values$BGEDworking), useTypes = FALSE, rowHeaders = FALSE)
  )
...
#reacing and cleaning of blue grass Social media data
  BGSMSdata <- observe({
    ...
    if (is.null(inFile))
      return(NULL)
    BGSMSfinal <- cleanBGSMS(inFile$datapath, prev.resp.id=last.max.bg)
...
#displaying of SMS raw data  
  output$BGSMSrawdata <- renderRHandsontable(
    rhandsontable(rbind(BGInitialList$SMS,values$BGSMSworking), useTypes = FALSE, rowHeaders = FALSE)
  )
...
is.nan.data.frame <- function(x)
  ...
  Feedbacktable <- reactive({
    ...
    if (is.null(inFile))
      return(NULL)
 #read in and create month and quarter columns
    PBfeedback <- read.csv(inFile$datapath,header=TRUE,strip.white = TRUE,blank.lines.skip = TRUE) %>%
