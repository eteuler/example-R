...
makePretty <- function(df,site.name,bkcolor){
  ...
  column_spec(0:1, bold = T,width="18em") %>%
  column_spec(2:10,width="3em") %>%
...
makePretty(df = df.hq,site.name = "Headquarters",bkcolor = "blue")
...
df.pb <- df.pb %>% 
  select(-Event.or.distribution.type.)
makePretty(df = df.pb,site.name = "Pueblo",bkcolor ="cyan")
...
df.bg <- df.bg %>%
  select(-Event.or.distribution.type.)
makePretty(df=df.bg, site.name = "Blue Grass", bkcolor = "green")
...
kable(df.feedback, "latex") %>% 
  kable_styling(latex_options = "striped", full_width = F, font_size = 8) %>% 
  row_spec(0:0, bold = T, color = "white", background = "black")
